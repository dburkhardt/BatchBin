...
def initializeArgparse():
    ...
    parser.add_argument(
        '--barcode_file',
        default = '~/barcode_table.tsv',
        help='the barcode file'
    )
    parser.add_argument(
        'samples', nargs='+',
        help='a list of codes of the form PHO (Prospect hill Heated)'
    )
    parser.add_argument(
        '-b','--bamdir',dest='bamdir', default= '~/bamfiles/',
    ...
    parser.add_argument(
        ...
        help= (
            "path to assembly.fa, default is "
...
#take a string ID like 'HOB' or 'PO' and returns a list of the barcodes 
#(e.g. ['AAATTT','GGGCCC'])
def get_barcodes( subset , barcode_table_asList ):
    subset_list = list(subset) #turns 'MPH' into ['M','P','H']
    subset_of_barcode_table = barcode_table_asList 
    for code in subset_list:
        subset_of_barcode_table = [
            record for record in subset_of_barcode_table for entry in record
            if re.search(code,entry) and not re.search('[ACTG]{6}',entry)
        ]
    return [record[0] for record in subset_of_barcode_table] 
    #returns only the first column of the table

#returns a single Popen object running samtools merge on a set of bamfiles
def mergeBamFilesPopen(list_of_barcodes, subset):
    list_of_bamfiles = [
        '%s%s.bam' % (bamdir,barcode) for barcode in list_of_barcodes
    ]
    if not os.path.exists('./mergedBamfiles/%s.tmp.bam' % subset):
    #check to see if this particular set of files has been created
        return subprocess.Popen(
            'samtools merge ./mergedBamfiles/%s.tmp.bam ' % subset +
            ' '.join(list_of_bamfiles), shell=True
        )
...
def indexBamFilesPopen(list_of_barcodes, subset):
    ...
    if not os.path.exists('./mergedBamfiles/%s.tmp.bam.bai' % subset): 
    #check to see if this particular set of files has been created
        return subprocess.Popen(
            'samtools index ./mergedBamfiles/%s.tmp.bam' % subset, shell=True
        )

#spawns a bunch of samtools merge subprocesses 
#and writes merged bamfiles to rundir/mergedBamfiles/ (BLOCKING)
def mergeBamfiles(samples, barcode_table_asList):
    print "Merging bamfiles"
    os.system('mkdir -p ./mergedBamfiles')
    list_of_bamfiles = [
        './mergedBamfiles/'+sample+'.tmp.bam' for sample in samples
    ]
    print list_of_bamfiles
    try:
        print [subset for subset in samples]
        print [
            get_barcodes(subset, barcode_table_asList) for subset in samples
        ]
        processes = [
            mergeBamFilesPopen(
                get_barcodes(subset, barcode_table_asList),subset
...
#returns a List of Popen objects running MetaBat 
#using the Specific, Sensitive, and Specific_Paired settings
def runMetaBat(list_of_samples, list_of_merged_bamfiles):
    ...
    os.system(
        "mkdir -p ./MetaBAT/Sensitive ./MetaBAT/Specific "
        "./MetaBAT/SpecificPair"
    )
    try:
        log_sensitive = open('./MetaBAT/Sensitive/MetaBat_sensititve.log','a')
        log_specific = open('./MetaBAT/Specific/MetaBat_specific.log','a')
        log_specific_paired = open(
            './MetaBAT/SpecificPair/MetaBat_SpecificPair.log','a'
        )
        p = [
            subprocess.Popen(
                " time nice -n 15 metabat -i %s -a MetaBat.depth.txt "
                "-o ./MetaBAT/Sensitive/bin --sensitive -v "
                "--saveTNF saved.tnf --saveDistance saved.gprob" % assembly,
        ...
        p.append(
            subprocess.Popen(
                "time nice -n 15 metabat -i %s -a MetaBat.depth.txt "
                "-o ./MetaBAT/Specific/bin --specific -v --saveTNF saved.tnf "
                "--saveDistance saved.gprob" % assembly,
        ...
        p.append(
            subprocess.Popen(
                "time nice -n 15 metabat -i %s -a MetaBat.depth.txt "
                "-p MetaBat.paired.txt -o ./MetaBAT/SpecificPair/bin "
                "--specific -v --saveTNF saved.tnf "
                "--saveDistance saved.gprob" % assembly, 
                stdout=log_specific_paired, stderr=log_specific_paired, 
...
def runConcoct():
    ...
    if not os.path.exists('./depth_concoct.txt'):
        subprocess.check_call(
            " awk \'NR > 1 {for(x=1;x<=NF;x++) if(x == 1 || "
            "(x >= 4 && x % 2 == 0)) printf \"%s\", $x (x == NF || "
            "x == (NF-1) ? \"\\n\":\"\\t\")}' ./MetaBat.depth.txt > "
            "./depth_concoct.txt", shell=True
        )
    else: 
        print "found Concoct depth file"
    call_to_source_concoct_env = (
        ". ~/software/anaconda/envs/concoct_env/bin/activate concoct_env"
        )
    call_to_bin_with_concoct = (
        "concoct --composition_file %s --coverage_file ../depth_concoct.txt" 
        % assembly
        )
    extract_bins = (
        "/home/dan/software/CONCOCT-0.4.0/scripts/extract_fasta_bins.py "
        "--output_path ./bins %s clustering_gt1000.csv" % assembly
        )
    concoct = subprocess.Popen(
        "cd Concoct ; %s ; time nice -n 15 %s ; time nice -n 15 %s" 
        % (call_to_source_concoct_env, call_to_bin_with_concoct, extract_bins),
        stdout=log_concoct, stderr=log_concoct, 
        shell=True, executable='/bin/bash'
        )
...
def runGroopM(list_of_bamfiles):
    ...
    parse = (
        "groopm parse -t 16 %s %s " % (database, assembly) + 
        ' '.join(list_of_bamfiles)
        )
    core = "groopm core %s " % database
    extract = (
        "groopm extract -t 32 --prefix ./GroopM/core_only/bin_groopm "
        "%s %s" % (database , assembly)
        )
    recruit = "groopm recruit %s" % database
    extract_recruit = (
        "groopm extract -t 32 --prefix ./GroopM/recruited/bin_groopm "
        "%s %s" % (database , assembly)
        )
    print "Binning using GroopM..."
    return subprocess.Popen(
        "time nice -n 15 %s ; time nice -n 15 %s ; time nice -n 15 %s ; "
        "time nice -n 15 %s ; time nice -n 15 %s" 
        % (parse, core, extract, recruit, extract_recruit),
        stdout = log_groopM, stderr = log_groopM, 
        shell = True, executable = '/bin/bash'
        )
...
def merge_and_run_binning_programs(samples, barcode_table_asList):
    ...
    try:
        ...
        return monitorProcesses_returnLast(all_processes)
    except KeyboardInterrupt:
...
if __name__ == '__main__':
    ...
    initializeVariables(argparser)
    barcode_table_asList = load_barcodeFile(argparser.barcode_file)
    merge_and_run_binning_programs(
        argparser.samples, barcode_table_asList).wait()
    print "Done binning!"
